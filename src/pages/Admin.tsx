...
const datasConvertidas = apolices.map(item => converterData(item.data))
    ...
    <div className="bg-white rounded-2xl border border-zinc-200 p-5 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h2 className="text-3xl font-bold text-zinc-800 mt-2">{value}</h2>
          ...
          >
            {positive ? <TrendingUp size={25} /> : <TrendingDown size={25} />}

            {percentage}
          </div>
        </div>

        <div className="bg-zinc-100 p-3 rounded-xl text-[#014d4e]">{icon}</div>
      </div>
    </div>
  )
}
...
export default function AdminDashboard() {
  ...
  const [dataInicial, setDataInicial] = useState(formatarInputDate(menorData))
  const [dataFinal, setDataFinal] = useState(formatarInputDate(maiorData))
  ...
  const lineData = meses.map((mes, index) => ({
    ...
    apolices: apolicesFiltradas.filter(
      item => Number(item.data.split('/')[1]) - 1 === index
    ).length,
        ...
        >
          <div>
            ...
            <div className={`mb-15 transition-all duration-500`}>
              <h1 className="text-3xl font-bold whitespace-nowrap">VITTALIS</h1>
          ...
          ${item === 'Dashboard' ? 'bg-white/10' : 'hover:bg-white/5'}
            `}
                >
          ...
          <div>
            <button
              className="mt-25 bg-orange-500/60 w-full flex items-center gap-3 px-5 py-4 rounded-xl transition-all duration-300
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              {usuario.foto ? (
                ...
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#d8ece7] font-bold text-[#014d4e]">
                  <User size={22} />
                </div>
              )}

              <div>
                <p className="font-semibold text-zinc-800">{nomeAdmin}</p>
        ...
        <section className="p-8">
          ...
          <div className="grid grid-cols-4 gap-5 mb-6">
            <StatsCard
              ...
              value={apolicesFiltradas.length}
              percentage="8,2%"
              positive
              icon={<Users />}
            />
                  ...
                  >
                    ...
                    {tipos.map((tipo, index) => (
                      <Line
                        ...
                        dataKey={tipo}
                        stroke={
                          ['#014d4e', '#2c8c7b', '#3962e6', '#e63939'][index]
                        }
                        strokeWidth={3}
                        dot={{ r: 2 }}
                        activeDot={{ r: 5 }}
              ...
              <div className="space-y-2 mt-2 p-10">
                {tipos.map((tipo, index) => (
                  <div key={tipo} className="flex items-center justify-between">
                    <div
                      onClick={() => setAtivo(ativo === tipo ? null : tipo)}
                      className={`flex items-center gap-3 hover: cursor-pointer transition-opacity duration-100 ${ativo === null ? 'opacity-100' : ativo === tipo ? 'opacity-100' : 'opacity-40'}`}
                    >
                      <div
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 p-6">
            <div className="flex items-center justify-between mb-9">
              <h2 className="text-xl font-semibold">Apólices recentes</h2>
